  ...
  CREATE OR REPLACE  VIEW EF2.V_DAILY_REPORT_STATUS_ALL_JOBS (BIZ_DT, TOTAL_NO_DAILY_JOBS, TOTAL_NO_SUCCESSFUL_JOBS, TOTAL_NO_INPROGRESS_JOBS, TOTAL_NO_FAILED_JOBS) AS 
...
TOTAL_NO_INPROGRESS_JOBS ,'F' TOTAL_NO_FAILED_JOBS )) order by biz_dt;
  ...
  CREATE OR REPLACE  VIEW EF2.V_FAILURE_REPORT (BIZ_DT, COUNTRY_CODE, JOB_NAME, PROCESS_ID, FILE_NAME, START_TIME, END_TIME, STATUS, FAILURE_REASON) AS 
  SELECT
...
FROM EDAG_PROCESS_LOG AA
 ...
 CREATE OR REPLACE  VIEW EF2.V_FILE_RECEPTION (COUNTRY_CODE, BIZ_DT, SOURCE_SYSTEM, FILE_FREQUENCY, FILE_TYPE, FILE_NAME, DATE_TIME_FILE_ARRIVAL, ROWS_INPUT, ROWS_INSERTED) AS 
  select
...
CREATE OR REPLACE  VIEW EF2.V_LOADING_COMP_BIZ (COUNTRY_CODE, SOURCE_SYSTEM, FILE_NAME, PROC_START_DT, DATE_TIME_FILE_ARRIVAL, HIVE_TABLE_NAME, DATE_TIME_HIVE_TABLE_LOAD, LOADING_STATUS, BIZ_DT, ROWS_INPUT, ROWS_INSERTED) AS 
  SELECT
...
order by file_name,proc_start_dt,biz_dt );
 ...
 CREATE OR REPLACE  VIEW EF2.V_LOADING_COMP_PS (COUNTRY_CODE, SOURCE_SYSTEM, FILE_NAME, PROC_START_DT, DATE_TIME_FILE_ARRIVAL, HIVE_TABLE_NAME, DATE_TIME_HIVE_TABLE_LOAD, CURRENT_LOADING_STAGE, LOADING_STATUS, BIZ_DT, ROWS_INPUT, ROWS_INSERTED) AS 
  SELECT
  country_code, source_system, file_name,proc_start_dt,DATE_TIME_FILE_ARRIVAL,
