...
and not exists (select 1 from edag_field_displaydeny_detail edfd where edfd.src_sys_nm = epm.src_sys_cd and lower(elp.tgt_tbL_nm) = lower(edfd.hive_tbl_nm) and lower(efid.fld_nm) = lower(edfd.fld_nm))
and ( epm.src_sys_cd in ('BWC', 'GRP', 'SSS', 'CEL', 'CFR', 'EPM', 'GCF') /* batch 1 */
    or epm.src_sys_cd in ('ARF', 'BIS' , 'FIS', 'FIT', 'LES', 'LNS', 'VBM') /* batch 2 */
    or epm.src_sys_cd in ('TAS') and efd.file_nm not like '%\_MY' escape '\' )
